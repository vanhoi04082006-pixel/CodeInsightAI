  ...
  if (!deep && !aiEnh) {
    return (
      <GlassCard className="p-8 text-center">
        ...
        <h3 className="mt-3 text-sm font-semibold">{t("reports", "aiInsights.staticOnly")}</h3>
        <p className="mt-1 text-xs text-muted-foreground">
          {t("reports", "aiInsights.staticOnlyDesc")}
        </p>
      </GlassCard>
    );
  }
      ...
      <div className="flex items-center gap-2">
        {deep?.badge === "deep-ai" ? (
          <span className="flex items-center gap-1.5 rounded-full bg-violet-500/15 px-3 py-1 text-xs font-medium text-violet-300">
            <Sparkles className="h-3.5 w-3.5" /> {t("reports", "aiInsights.deepAI")}
          </span>
        ) : (
          <span className="flex items-center gap-1.5 rounded-full bg-cyan-500/10 px-3 py-1 text-xs font-medium text-cyan-300">
            <Activity className="h-3.5 w-3.5" /> {t("reports", "aiInsights.aiEnhanced")}
          </span>
        )}
      </div>

      <GlassCard className="p-6">
        <h3 className="flex items-center gap-2 text-sm font-semibold"><Sparkles className="h-4 w-4 text-violet-300" /> {t("reports", "aiInsights.execSummary")}</h3>
      ...
      {deep && (
        <>
          {deep.securityReview?.length > 0 && (
            <GlassCard className="p-6">
              <h3 className="flex items-center gap-2 text-sm font-semibold"><ShieldCheck className="h-4 w-4 text-rose-400" /> {t("reports", "aiInsights.securityReview")}</h3>
              <div className="mt-3 space-y-3">
                {deep.securityReview.map((s: any, i: number) => (
                  <div key={i} className="rounded-lg border border-rose-500/15 bg-rose-500/[0.03] p-3">
                    ...
                    <p className="mt-1 text-xs text-muted-foreground"><span className="font-medium">{t("reports", "aiInsights.rootCause")}:</span> {s.rootCause}</p>
                    <p className="mt-1 text-xs text-muted-foreground"><span className="font-medium">{t("reports", "aiInsights.impact")}:</span> {s.impact}</p>
          ...
          {deep.architectureReview && (
            <GlassCard className="p-6">
              <h3 className="flex items-center gap-2 text-sm font-semibold"><Network className="h-4 w-4 text-cyan-300" /> {t("reports", "aiInsights.archReview")}</h3>
          ...
          {deep.performanceReview?.length > 0 && (
            <GlassCard className="p-6">
              <h3 className="flex items-center gap-2 text-sm font-semibold"><Zap className="h-4 w-4 text-amber-400" /> {t("reports", "aiInsights.perfReview")}</h3>
              <div className="mt-3 space-y-3">
                {deep.performanceReview.map((p: any, i: number) => (
                  <div key={i} className="rounded-lg border border-amber-500/15 bg-amber-500/[0.03] p-3">
                    ...
                    <p className="mt-1 text-xs text-muted-foreground"><span className="font-medium">{t("reports", "aiInsights.rootCause")}:</span> {p.rootCause}</p>
                    ...
                    <p className="mt-1 text-xs text-emerald-300"><span className="font-medium">{t("reports", "aiInsights.expected")}:</span> {p.expectedImprovement}</p>
                  </div>
                ))}
              </div>
            </GlassCard>
          ...
          {deep.bestPracticesAudit && (
            <GlassCard className="p-6">
              <h3 className="flex items-center gap-2 text-sm font-semibold"><Network className="h-4 w-4 text-cyan-300" /> {t("reports", "aiInsights.bestPractices")} — {deep.bestPracticesAudit.framework}</h3>
              ...
              {deep.bestPracticesAudit.passed?.length > 0 && (
                <div className="mt-3">
                  <p className="text-[10px] uppercase tracking-wider text-emerald-400 mb-1">{t("reports", "aiInsights.passed")}</p>
              ...
              {deep.bestPracticesAudit.failed?.length > 0 && (
                <div className="mt-3 space-y-2">
                  <p className="text-[10px] uppercase tracking-wider text-rose-400 mb-1">{t("reports", "aiInsights.needsImprovement")}</p>
          ...
          {deep.priorities?.length > 0 && (
            <GlassCard className="p-6">
              <h3 className="flex items-center gap-2 text-sm font-semibold"><Zap className="h-4 w-4 text-amber-400" /> {t("reports", "aiInsights.priorities")}</h3>
              <div className="mt-3 space-y-2">
                {deep.priorities.map((p: any, i: number) => (
                  <div key={i} className="flex items-start gap-2 rounded-lg border border-white/5 bg-white/[0.02] p-3">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-muted-foreground"><span className="font-medium">{t("reports", "aiInsights.impact")}:</span> {p.businessImpact}</p>
                      <p className="text-[11px] text-muted-foreground"><span className="font-medium">{t("reports", "aiInsights.recommendation")}:</span> {p.recommendation}</p>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {deep.roadmap?.length > 0 && (
            <GlassCard className="p-6">
              <h3 className="flex items-center gap-2 text-sm font-semibold"><Rocket className="h-4 w-4 text-cyan-300" /> {t("reports", "aiInsights.roadmap")}</h3>
              <div className="mt-3 space-y-3">
