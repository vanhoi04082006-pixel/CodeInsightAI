    ...
    <div className="relative flex min-h-screen items-center justify-center overflow-hidden px-4">
      ...
      >
        <GlassCard strong className="p-8 text-center md:p-10">
          ...
          {/* Google Login Button (optional — only shows if GOOGLE_ID is set) */}
          <motion.button
            whileHover={{ scale: redirecting ? 1 : 1.02 }}
            whileTap={{ scale: redirecting ? 1 : 0.98 }}
            onClick={() => {
            ...
            disabled={redirecting}
            className="mt-2 flex w-full items-center justify-center gap-2.5 rounded-xl border border-white/10 bg-white/[0.03] px-4 py-3 text-sm font-medium text-foreground transition hover:bg-white/[0.06] disabled:cursor-not-allowed disabled:opacity-70"
          >
          ...
          <p className="mt-1 text-center text-[10px] text-muted-foreground">
            Google users can analyze public repos. Link GitHub for private repos.
          </p>
