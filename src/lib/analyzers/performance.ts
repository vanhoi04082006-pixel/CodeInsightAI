...
// 24 rules: bundle, React, async, Next.js, query, blocking
...
export function analyzePerformance(files: { path: string; content: string }[]): Issue[] {
  ...
  for (const { path: fpath, content } of files) {
    ...
    if (content.match(/import\s+.*from\s+['"]underscore['"]/))
      issues.push(mk("bundle","underscore imported","underscore adds ~60KB. Use individual lodash/es modules or native JS.","medium","trivial"));
    // 4. Full rxjs import
    if (content.match(/import\s+\*\s+as\s+.*from\s+['"]rxjs['"]/))
      issues.push(mk("bundle","Full rxjs import","Importing all of rxjs is huge. Use 'import { map } from rxjs/operators'.","high","medium"));
    // 5. antd full import
    if (content.match(/import\s+.*from\s+['"]antd['"]/) && !content.includes("ConfigProvider"))
      issues.push(mk("bundle","antd full import without tree-shaking","antd may bundle unused components. Use babel-plugin-import or individual imports.","medium","medium"));
    ...
    if ((fpath.endsWith("index.ts") || fpath.endsWith("index.tsx")) && (content.match(/export\s+\*\s+from\s+['"].*['"]/g) || []).length > 3)
      issues.push(mk("bundle","Barrel file with many re-exports","index.ts re-exports many modules — prevents tree-shaking.","low","small"));
